...
const emit = defineEmits(["set-display", "set-input-display", "set-likes"]);

const commentButton = computed(() => {
  const length = props.postComments.comments.length;
  return length > 0
    ? `Showing ${
        props.postComments.display ? "" : `${length > 3 ? "3" : length} of `
      }${length} comments`
    : "";
});
</script>
...
<template>
  ...
  >
    <div class="d-flex">
      ...
      <IconButton
        :button-color="
          'btn-outline-' + (postComments.reply ? 'secondary' : 'primary')
        "
        :disabled-stat="guestStat"
