...
<template>
  <nav class="navbar navbar-expand-lg bg-light mb-2" id="nav-container">
    <div
      class="container-fluid d-flex align-items-center justify-content-between"
    >
      <RouterLink
        exact-active-class="exact-active"
        class="navbar-brand active ms-1 me-2"
        :to="'/home'"
      >
        Fakebook
      </RouterLink>
      ...
      <h5 class="m-0" id="welcome-text">
        Hello {{ userStore.getAllUsernames()[currentStore.userId] }}!
      </h5>
      <div v-if="992 <= screenSize" class="d-flex align-items-center">
        ...
        />
        <a class="nav-item nav-link active mx-1" href="https://www.ayeung.me">
          Portfolio
        </a>
        <RouterLink
          class="nav-item nav-link active mx-1"
          :to="'/welcome'"
          @click="currentStore.setUser(-1)"
        >
          Log out
        </RouterLink>
      </div>
