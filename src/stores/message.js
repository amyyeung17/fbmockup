...
export const useMessageStore = defineStore("message", () => {
  ...
  const getMsg = computed(() => {
    return (id, mid) =>
      getAllMsgs.value(id).find((v) => v.mid === parseInt(mid));
  });

  const getCurrentMsg = computed(() => {
    return (id) =>
      currentStore.userId !== -1
        ? getMsg.value(id, currentStore.recipientId)
        : [];
  });

  const getAllCurrent = computed(() => {
    return getAllMsgs.value(currentStore.userId);
  });

  function update(id) {
    if (
      getAllCurrent.value.length === 0 ||
      (getAllCurrent.value.length !== 0 &&
        [...getAllCurrent.value.map((s) => s.mid)].indexOf(id) === -1)
    ) {
  ...
  function enterMsg(content) {
    ...
    const recipient = getMsg.value(
      currentStore.recipientId,
      currentStore.userId
    );
