...
export const useMessageStore = defineStore({
  ...
  getters: {
    getMsg: (state) => {
      return(id, mid) => {
       return state.message.find(m => m.id === parseInt(id)).convo.find(v => v.mid === parseInt(mid))
      }
    },
    getCurrentMsg: (state) => {
      return (id) => {
        const currentStore = useCurrentStore();
        return state.getMsg(id, currentStore.currentMsg)
      };
    },
    getAllMsgs: (state) => {
      return (id) => {
        return state.message.find(m => m.id === parseInt(id)).convo
      }
    }
  },
  actions: {
    update(id) {
      const currentStore = useCurrentStore();
      const allMsg = this.getAllMsgs(currentStore.currentId)
      if (allMsg.length === 0 || (allMsg.length !== 0 && [...allMsg.map(s => s.mid)].indexOf(id) === -1)) {
        allMsg.push({ mid: id, msg: [] })
        this.getAllMsgs(id).push({ mid: currentStore.currentId, msg: [] })
      }
      currentStore.setMsg(id);
    },
    enterMsg(content) {
      const currentStore = useCurrentStore();
      const mes = this.getCurrentMsg(currentStore.currentId);
      const recipient = this.getMsg(currentStore.currentMsg, currentStore.currentId)
      if (content !== '') {
        mes.msg.push({time: mes.msg.length, user: currentStore.currentId, m: content});
        recipient.msg.push({time: mes.msg.length, user: currentStore.currentId, m: content})
      }
    },
    removeEmpty(currentId) {
      const currentMessage = this.getAllMsgs(currentId)
      if (currentMessage.length !== 0) {
        for (let j = 0; j < currentMessage.length; j++) {
          if (currentMessage[j].msg.length === 0) {
            currentMessage.splice(j, 1);
