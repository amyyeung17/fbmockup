    ...
    getFilterId: (state) => {
      return (id) => {
        return state.post.filter(
          (p) => p.id === parseInt(id) || p.postLocation === parseInt(id)
        );
      };
    },
    getFilterAll: (state) => {
      return () => {
        ...
        const friendsPosts = state.post.filter(
          (p) =>
            userFriends.indexOf(p.id) !== -1 ||
            (userFriends.indexOf(p.postLocation) !== -1 &&
              ![...userPosts.map((u) => u.pid)].includes(p.pid))
        );
